...
def stat_str(x):
    ...
    return f"{mean:6.2f} +/- {std:6.2f}  {x.shape}"
...
class Discriminator(nn.Module):
    def __init__(self, resolution):
        ...
        self.input_1 = nn.Identity(self.resolution, self.resolution, 3)
        self.image_conversion = ImageConversion(ImageConversionMode.TORCH_TO_MODEL)
        self.m_modules = nn.ModuleDict()
#         self.m_modules.update({"input_1", nn.Identity(self.resolution, self.resolution, 3)})
#         self.m_modules.update({"image_conversion", ImageConversion(ImageConversionMode.TORCH_TO_MODEL)})
        ...
        while res < self.resolution:
            ...
            self.m_modules.update({f"conv_{res}x{res}_2": conv_2d(out_channel_count=self.resolution_to_feature_counts[res][1],
                                                                in_channel_count=self.resolution_to_feature_counts[res][0],
                                                                stride=2)})
    ...
    def forward(self, x):
        ...
        image_conversion = self.image_conversion(input_1)
        x = self.m_modules[f"from_rgb_{self.resolution}x{self.resolution}"](image_conversion)

        res = self.resolution
        while res > 4:
            conv_1 = self.m_modules[f"conv_{res}x{res}_1"](x)
            conv_2 = self.m_modules[f"conv_{res}x{res}_2"](conv_1)
